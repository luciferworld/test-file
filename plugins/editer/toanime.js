      ...
      try {
         if (m.quoted ? m.quoted.message : m.msg.viewOnce) {
            ...
            if (/image/.test(type)) {
               ...
               const json = await Func.fetchJson(`https://api.betabotz.eu.org/api/maker/jadianime?url=${image.data.url}&apikey=beta-Ibrahim1209`)
                 if (!json.status) return m.reply(Func.jsonFormat(json))
               client.sendFile(m.chat, json.result.img_crop_single, 'image.jpg', '', m)
